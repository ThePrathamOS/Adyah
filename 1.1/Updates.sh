...
sed -i '56s~.*~~' /opt/essentials/PrathamOSUpGradeFull.sh
sed -i '127s~.*~~' /opt/essentials/PrathamOSUpGradeFull.sh
sed -i '128s~.*~~' /opt/essentials/PrathamOSUpGradeFull.sh
sed -i '72s~.*~~' /opt/essentials/PrathamOSUpGradeFull.sh
sed -i "72s~.*~DEBIAN_FRONTEND=noninteractive sudo apt-get install --reinstall -y firefox python3.6 ruby2.5 skypeforlinux teamviewer flatpak docker-ce snapd clementine vlc kodi kodi-bin liferea python3.6-minimal mousepad mpv baobab bash apache2 cmake cmake-qt-gui transmission-gtk uget amd64-microcode aria2 youtube-dl file-roller gscan2pdf hwinfo vlc-l10n vlc-plugin-notify vlc-plugin-samba vlc-plugin-video-splitter vlc-plugin-visualization whois wine-staging wine-staging-amd64 wine-staging-i386:i386 winehq-staging winetricks r-base nano r-base-dev r-cran-cluster r-cran-foreign r-cran-matrix r-cran-nlme r-cran-survival clamav clamtk clamav-base clamav-daemon clamav-freshclam clamdscan q4wine qemu qemu-kvm qemu-system qemu-system-common qemu-utils qt5-gtk-platformtheme remmina remmina-common remmina-plugin-exec remmina-plugin-nx remmina-plugin-rdp remmina-plugin-secret remmina-plugin-spice remmina-plugin-telepathy remmina-plugin-vnc remmina-plugin-xdmcp rpm rpm-common rpm2cpio rsyslog samba samba-common samba-common-bin samba-dsdb-modules samba-libs samba-vfs-modules usbview network-manager-gnome openssl fslint git vim vim-common vim-runtime vim-tiny bluetooth bluez bluez-cups bluez-obexd bluez-tools cpp-7 cups cups-client cups-common cups-core-drivers cups-daemon cups-ipp-utils cups-ppdc cups-server-common dbus dbus-user-session dbus-x11 lrzip pxz cpp-7 g++-7 gcc-6-base gcc-7 gcc-7-base gcc-8-base gcc-8-base:i386 lightdm \&\& echo 'NBLABLAn' | sudo apt-get install --reinstall lightdm-gtk-greeter -y~" /opt/essentials/PrathamOSUpGradeFull.sh
sed -i -e 's/BLABLA/\\/' /opt/essentials/PrathamOSUpGradeFull.sh

sudo -H -u root bash -c 'sed -i "'"2s~.*~~"'" /root/setupboot.sh'
sudo -H -u root bash -c 'sed -i "'"2s~.*~echo \"adyah\" | sudo switchenv~"'" /root/setupboot.sh'
sudo -H -u root bash -c 'sed -i "'"48s~.*~~"'" /root/CreateUser.sh'
sudo -H -u root bash -c 'sed -i "'"48s~.*~\tfind /home/DOLLARNewUserName -type f -exec sed -i \"s/primaryuser/DOLLARNewUserName/g\" {} + \&\& chown -R DOLLARNewUserName:DOLLARNewUserName /home/DOLLARNewUserName~"'" /root/CreateUser.sh'
sudo -H -u root bash -c 'sed -i -e "'"s/DOLLAR/$/g"'" /root/CreateUser.sh'
pushd /home
for DIR in * ; do
    	if [ "$DIR" == "linuxbrew" ] || [ "$DIR" == "linuxbrew" ] ; then
	:
	else
		sudo -H -u root bash -c "find /home/$DIR -type f -exec sed -i \"s/primaryuser/$DIR/g\" {} +"
	fi
done
popd
