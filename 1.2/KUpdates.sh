...
axel https://sourceforge.net/projects/getprathamos/files/Adyah/1.2/Kernel.7z
FILE="/opt/KernelUpdate/Kernel.7z"
if [ -f "$FILE" ]
then
	md5sum=$(md5sum $FILE | awk '{print $1}')
	if [ "$md5sum" = "27784cb97ef0f5fcb16fc7c447718484" ]; then
		TOTAL="1234"
		if [ "$TOTAL" = "1234" ]; then
			...
			KERNELNAME="4.18.0-041800rc4-generic"
			sudo gdebi -n linux-modules-4.18.0-041800rc4-generic_4.18.0-041800rc4.201807082030_amd64.deb
			sudo gdebi -n linux-headers-4.18.0-041800rc4_4.18.0-041800rc4.201807082030_all.deb
			sudo gdebi -n linux-headers-4.18.0-041800rc4-generic_4.18.0-041800rc4.201807082030_amd64.deb
			sudo gdebi -n linux-image-unsigned-4.18.0-041800rc4-generic_4.18.0-041800rc4.201807082030_amd64.deb
			CurrentUser=$(whoami)
